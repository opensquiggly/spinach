...
public class Parser
{
  ...
  // Unexpected error
  private static String ERR_INTERNAL_ERROR = "regexp/syntax: internal error";

  // Parse errors
  //private static String ERR_INVALID_CHAR_CLASS = "invalid character class";
  private static String ERR_INVALID_CHAR_RANGE = "Invalid character class range";
  private static String ERR_INVALID_ESCAPE = "Invalid escape sequence";
  private static String ERR_INVALID_NAMED_CAPTURE = "Invalid named capture";
  private static String ERR_INVALID_PERL_OP = "Invalid or unsupported Perl syntax";
  private static String ERR_INVALID_REPEAT_OP = "Invalid nested repetition operator";
  private static String ERR_INVALID_REPEAT_SIZE = "Invalid repeat count";
  private static String ERR_MISSING_BRACKET = "Missing closing ]";
  private static String ERR_MISSING_PAREN = "Missing closing )";

  private static String ERR_MISSING_REPEAT_ARGUMENT =
    "missing argument to repetition operator";

  private static String ERR_TRAILING_BACKSLASH = "trailing backslash at end of expression";

  private String wholeRegexp;

  // Flags control the behavior of the parser and record information about
  // regexp context.
  private int _flags; // parse mode flags

  // Stack of parsed expressions.
  private List<Regexp> stack = new List<Regexp>();
  private Regexp free;
  ...
  private Regexp pop()
  {
    Regexp result = stack[stack.Count - 1];
  ...
  private Regexp[] popToPseudo()
  {
    ...
    while (i > 0 && !Regexp.isPseudo(stack[i - 1].op))
    {
      i--;
    }

    List<Regexp> subrange = stack.GetRange(i, n - i);
    Regexp[] r = subrange.ToArray();
    stack.RemoveRange(i, n - i);
    return r;
  }
  ...
  private Regexp push(Regexp re)
  {
    if (re.op == Regexp.Op.CHAR_CLASS && re.runes.Length == 2 && re.runes[0] == re.runes[1])
    {
      ...
      if (maybeConcat(re.runes[0], _flags & ~RE2.FOLD_CASE))
      {
        return null;
      }
  ...
  private bool maybeConcat(int r, int flags)
  {
    ...
    Regexp re1 = stack[n - 1];
    Regexp re2 = stack[n - 2];
  ...
  private void literal(int r) => push(newLiteral(r, _flags));
  ...
  private Regexp collapse(Regexp[] subs, Regexp.Op op)
  {
    ...
    var newsubs = new Regexp[len];
    int i = 0;
    foreach (Regexp sub in subs)
    {
      if (sub.op == op)
      {
        System.Array.Copy(sub.subs, 0, newsubs, i, sub.subs.Length);
        i += sub.subs.Length;
        reuse(sub);
      }
      else
  ...
  // reuse.
  public Regexp removeLeadingRegexp(Regexp re, bool again)
  {
    if (re.op == Regexp.Op.CONCAT && re.subs.Length > 0)
    {
      if (again)
  ...
  {
    private String str; // a stream of UTF-16 codes
    ...
    public bool more() => _pos < str.Length;

    // Returns the rune at the cursor position.
    // Precondition: |more()|.
    public int peek() => Char.ConvertToUtf32(str, pos());//               return str[pos()];//                return str.codePointAt(pos);
  ...
  {
    if ((_flags & RE2.LITERAL) != 0)
    {
      // Trivial parser for literal string.
      return literalRegexp(wholeRegexp, _flags);
    }
    ...
    while (t.more())
    {
      ...
      switch (t.peek())
      {
        ...
        case '(':
          if ((_flags & RE2.PERL_X) != 0 && t.lookingAt("(?"))
          {
        ...
        case '\\':
          {
            int savedPos = t.pos();
            t.skip(1); // '\\'
            bool done_with_bigswitch = false;
            if ((_flags & RE2.PERL_X) != 0 && t.more())
            {
              int c = t.pop();
              switch (c)
              {
            ...
            {
              var cc1 = new CharClass();
              if (parseUnicodeClass(t, cc1))
              {
                re.runes = cc1.ToArray();
                push(re);
                break;
              }
            }
  ...
  private static int parseInt(StringIterator t)
  {
    ...
    if (n.Length == 0 || (n.Length > 1 && n[0] == '0'))
    {
    ...
    return Int32.Parse(n); // can't fail
  }
  ...
  private bool swapVerticalBar()
  {
    ...
    int n = stack.Count;
    if (n >= 3
        && stack[n - 2].op == Regexp.Op.VERTICAL_BAR
        && isCharClass(stack[n - 1])
        && isCharClass(stack[n - 3]))
    {
      Regexp re1 = stack[n - 1];
      Regexp re3 = stack[n - 3];
  ...
  private bool parsePerlClassEscape(StringIterator t, CharClass cc)
  {
    ...
    CharGroup.PERL_GROUPS.TryGetValue(t.from(beforePos), out CharGroup g);
    if (g == null)
    {
      return false;
    }

    cc.AppendGroup(g, (_flags & RE2.FOLD_CASE) != 0);
    return true;
  }
  ...
  private bool parseNamedClass(StringIterator t, CharClass cc)
  {
    ...
    t.skipString(name);
    CharGroup.POSIX_GROUPS.TryGetValue(name, out CharGroup g);
    if (g == null)
    {
  ...
  public static Pair<int[][], int[][]> unicodeTable(String name)
  {
    ...
    UnicodeTables.CATEGORIES.TryGetValue(name, out int[][] table);
    if (table != null)
    {
      UnicodeTables.FOLD_CATEGORIES.TryGetValue(name, out int[][] value);
      Pair<int[][], int[][]>.of(table, value);
    }

    UnicodeTables.SCRIPTS.TryGetValue(name, out int[][] t2);
    table = t2;
    if (table != null)
    {
      UnicodeTables.FOLD_SCRIPT.TryGetValue(name, out int[][] value);
      return Pair<int[][], int[][]>.of(table, value);
    }
  ...
  {
    int startPos = t.pos();
    if ((_flags & RE2.UNICODE_GROUPS) == 0 || (!t.lookingAt("\\p") && !t.lookingAt("\\P")))
    {
      return false;
    }
    ...
    if ((_flags & RE2.FOLD_CASE) == 0 || fold == null)
    {
  ...
  private void parseClass(StringIterator t)
  {
    ...
    while (!t.more() || t.peek() != ']' || first)
    {
      ...
      if (t.more() && t.lookingAt('-') && (_flags & RE2.PERL_X) == 0 && !first)
      {
        String s = t.rest();
  ...
  private static int[] concatRunes(int[] x, int[] y)
  {
    ...
    System.Array.Copy(x, 0, z, 0, x.Length);
    System.Array.Copy(y, 0, z, x.Length, y.Length);
    return z;
  }
}
